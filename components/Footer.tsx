    ...
    <footer className="facet streak mt-3xl border-t-4 border-accent bg-deep text-fg">
      <div className="mx-auto grid max-w-[1180px] gap-xl px-md py-2xl sm:grid-cols-[1fr_auto]">
        <div>
          ...
          <p className="mt-md max-w-[46ch] font-mono text-micro uppercase tracking-wider text-muted">
            {site.tagline}
          </p>
        ...
        <nav aria-label="フッター" className="font-mono text-micro uppercase tracking-wider">
          <ul className="space-y-sm">
            {SECTIONS.map((section) => (
              <li key={section.slug}>
                <Link href={`/${section.slug}`} className="hover:text-accent-hot">
                  {section.label}
                </Link>
              </li>
            ))}
            <li>
              <Link href="/about" className="hover:text-accent-hot">
                STEPWIREについて
              </Link>
            </li>
            <li>
      ...
      <div className="border-t border-line-strong">
        <p className="mx-auto max-w-[1180px] px-md py-md font-mono text-micro uppercase tracking-wider text-faint">
