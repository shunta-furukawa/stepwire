...
ファイルは MP3 128 kbps に落として置く（`ffmpeg -i in.mp3 -c:a libmp3lame -b:a 128k out.mp3`）。
RMS は変わらないので `gain` はそのまま。320 kbps のままだと 1 曲 12 MB になり、
`pnpm content:validate` の上限（8 MB）で止まる。ベッド BGM は打鍵音の下で 0.3 前後に絞られ、
書き出しと YouTube でさらに二度圧縮されるので、128 kbps で差は出ない。AAC のほうが小さいが、
オープンソース版の Chromium は AAC を復号できないので、どこでも鳴る MP3 にしている。
